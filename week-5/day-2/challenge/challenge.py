...
class Gene:
    ...
    def __repr__(self):
        return f'im a gene with the value of {self.state}'
...
class Organism:
    def __init__(self,Dna,environment = 0.50):
        self.dna = Dna
        self.probabilty  = environment
        # if environment < random.randint(0,100):

    def mutate(self):
        if self.probabilty < random.randint(0,100):
            self.dna.mutate()
